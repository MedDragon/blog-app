...
const isAuthWallOpen = ref(false); // Для гостей
...
<template>
    ...
    <AuthenticatedLayout>
                ...
                >
                    <template #edit-post-form>
                        <div class="bg-slate-900/80 p-6 rounded-3xl border border-indigo-500/30">
                            <input v-model="editPostForm.title" class="w-full bg-slate-950 text-white text-xl font-black border-white/10 rounded-xl mb-3" />
                            <textarea v-model="editPostForm.content" rows="6" class="w-full bg-slate-950 text-slate-300 border-white/10 rounded-2xl p-4"></textarea>
                            <div class="flex gap-3 mt-4">
                                <button @click="submitEditPost(post.id)" class="bg-indigo-600 text-white px-8 py-3 rounded-xl text-[10px] font-black uppercase">Зберегти</button>
                                <button @click="editingPostId = null" class="text-slate-500 text-[10px] font-black uppercase">Скасувати</button>
                            </div>
                        </div>
                    </template>
                </PostItem>
        ...
        <button @click="checkGuest(() => isPostModalOpen = true)" class="fixed bottom-10 right-10 z-50 bg-indigo-600 text-white px-10 h-16 rounded-full shadow-2xl flex items-center gap-3 hover:scale-105 active:scale-95 transition-all">
        ...
        <div v-if="isAuthWallOpen" class="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-slate-950/95 backdrop-blur-2xl">
            <div class="bg-slate-900 border border-indigo-500/30 w-full max-w-md rounded-[3.5rem] p-12 text-center shadow-2xl animate-in">
                <div class="w-20 h-20 bg-indigo-600 rounded-3xl flex items-center justify-center mx-auto mb-8 shadow-indigo-500/50 shadow-2xl">
                    <span class="text-white text-4xl font-black italic">P</span>
                </div>
                <h3 class="text-3xl font-black text-white uppercase mb-4 italic tracking-tighter">Хочеш у Prostir?</h3>
                <p class="text-slate-400 text-sm mb-10 leading-relaxed font-bold uppercase tracking-widest opacity-60">Взаємодія доступна лише учасникам спільноти.</p>
                <div class="space-y-4">
                    <Link :href="route('login')" class="block w-full bg-white text-black py-5 rounded-2xl font-black uppercase text-xs tracking-widest hover:bg-slate-200 transition">Увійти</Link>
                    <Link :href="route('register')" class="block w-full bg-indigo-600 text-white py-5 rounded-2xl font-black uppercase text-xs tracking-widest hover:bg-indigo-500 transition">Реєстрація</Link>
                    <button @click="isAuthWallOpen = false" class="text-slate-500 text-[10px] font-black uppercase tracking-widest hover:text-white transition">Продовжити перегляд</button>
                </div>
            </div>
        </div>

        <div v-if="isPostModalOpen" class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/90 backdrop-blur-xl">
            <div class="bg-slate-900 border border-white/10 w-full max-w-2xl rounded-[3rem] p-12 shadow-2xl animate-in">
                <h3 class="text-3xl font-black text-white uppercase mb-8 italic">Новий пост</h3>
                <form @submit.prevent="submitPost" class="space-y-6">
                    <input v-model="postForm.title" placeholder="Заголовок" class="w-full bg-slate-950 border-white/10 rounded-2xl p-5 text-white focus:ring-indigo-500" />
                    <textarea v-model="postForm.content" placeholder="Контент..." rows="6" class="w-full bg-slate-950 border-white/10 rounded-2xl p-5 text-slate-300 focus:ring-indigo-500"></textarea>
                    <div class="flex justify-end gap-4">
                        <button type="button" @click="isPostModalOpen = false" class="px-8 py-4 text-slate-500 font-bold uppercase text-xs hover:text-white transition">Скасувати</button>
                        <button type="submit" class="bg-indigo-600 text-white px-10 py-4 rounded-2xl font-black uppercase text-xs shadow-xl hover:bg-indigo-500 transition">Опублікувати</button>
                    </div>
                </form>
            </div>
        </div>
        ...
        <div v-if="isCommentModalOpen" class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/90 backdrop-blur-xl">
            <div class="bg-slate-900 border border-white/10 w-full max-w-xl rounded-[3rem] p-12 shadow-2xl animate-in">
                <div v-if="replyPreview" class="mb-8 p-5 bg-indigo-500/10 rounded-3xl border border-indigo-500/20 text-xs">
                    <div class="font-black text-indigo-400 uppercase mb-1">Відповідь для {{ replyPreview.user }}</div>
                    <div class="text-slate-400 italic">"{{ replyPreview.text }}"</div>
                </div>
                <form @submit.prevent="submitComment">
                    ...
                    <div class="flex justify-end gap-4">
                        ...
                        <button type="submit" class="bg-white text-black px-10 py-4 rounded-2xl font-black uppercase text-xs hover:bg-indigo-600 hover:text-white transition">Надіслати</button>
                    </div>
                </form>
            </div>
        </div>
